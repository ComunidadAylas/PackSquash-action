...
export class PackSquashOptions {
  ...
  public static async parseAndTweak(workingDirectory: WorkingDirectory) {
    ...
    try {
      ...
      info(
        "The specified PackSquash options string could not be opened as a file. Treating it as a TOML string instead",
      );
    ...
    return new PackSquashOptions(options, TOML.stringify(options));
  }
  ...
  mayCacheBeUsed() {
    const neverStoreTimes =
      typeof this.options.never_store_squash_times === "boolean" ? this.options.never_store_squash_times : false;
    const zipConformanceLevel =
      typeof this.options.zip_spec_conformance_level === "string"
        ? this.options.zip_spec_conformance_level
        : "pedantic";
