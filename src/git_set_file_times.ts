...
async function setPackFilesModificationTimesFromCommits(workspace: string, packDirectory: string) {
  ...
  for (const candidateRepositoryPath of [workspace, ...submodules]) {
    ...
    const isRelevant =
      isPathWithin(packDirectory, candidateRepositoryPath) || isPathWithin(candidateRepositoryPath, packDirectory);
...
async function setPackFilesModificationTime(repository: string, remainingPackFiles: Set<string>) {
  ...
  const gitOut = await getExecOutput(
    "git",
    ["-C", repository, "log", "--diff-merges=cc", "--name-only", "--pretty=format:%ct", "-z"],
    {
      silent: true,
    },
  );
  ...
  let logEntryMatch: RegExpExecArray | null;
  let lastLogEntryMatchIndex: number | undefined;
  ...
  if (lastLogEntryMatchIndex !== undefined && lastLogEntryMatchIndex !== gitLog.length) {
    warning(
      `Could not parse the entire Git log for the repository at ${repository} (${lastLogEntryMatchIndex}/${gitLog.length} bytes parsed). Caching effectiveness may be degraded. Please report an issue`,
    );
  }
}
