...
import { getEnvOrThrow, extractFirstFileFromZip } from './util';
...
export class PackSquashBinaryManifest {
    ...
    public static async fetchManifest() {
        ...
        if ('ACT' in process.env) {
            ...
            try {
                const httpResponse = await new HttpClient().get(`https://binarymanifests.action.packsquash.aylas.org/v${CURRENT_MANIFEST_VERSION}`);
                manifestJson = await httpResponse.readBody();
            } catch (err) {
                warning(
                    `Could not retrieve the latest PackSquash binary manifest: ${err}. ` +
                        'The action will fall back to a default manifest, but this may cause errors. ' +
                        'If this is not caused by a transient network issue, please update the action or report the problem'
                );
    ...
    public async download(version: string, workingDirectory: WorkingDirectory) {
        ...
        if (version.startsWith('v')) {
            if (this.latestRelease != version) {
                warning(
                    `PackSquash ${this.latestRelease} is available, but this workflow is using ${version}. ` +
                        'Please update it at your earliest convenience to enjoy better support and the latest changes'
                );
            }
    ...
    private async downloadUnstable(commit: string, workingDirectory: WorkingDirectory) {
        ...
        const manifest = this.unstable.find(manifest => {
            ...
            return applicabilityPeriod.from <= commitTimestamp && (applicabilityPeriod.to == 'now' || applicabilityPeriod.to >= commitTimestamp);
        });
        if (manifest === undefined) {
    ...
    private async downloadFromManifest(manifest: BuildManifest, workingDirectory: WorkingDirectory, commit?: string) {
        ...
        let runArtifacts;
        let runArtifact;
        let artifactArchiveUrl;
        switch (manifest.fetch.strategy) {
            case 'httpGetRequest':
                resolvedUrl = resolve(manifest.fetch.url);
                ...
                break;
            case 'actionsWorkflowArtifact':
                debug('Fetching workflow runs');

                workflowId = manifest.fetch.workflowId;
                artifactName = resolve(manifest.fetch.name);
                ...
                latestWorkflowRun = workflowRuns.filter(r => r.workflow_id == workflowId).sort((r, s) => s.run_number - r.run_number)[0];
                if (!latestWorkflowRun) {
                    throw new Error('Could not find a workflow run to fetch the specified PackSquash version from. Please try again later');
                }
                ...
                runArtifact = runArtifacts.find(artifact => artifact.name == artifactName);
                if (!runArtifact) {
