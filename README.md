...
| `github_token` | `${{ github.token }}` | The GitHub API authentication token that will be used for operations that may require authentication. A token is required to download the `latest` PackSquash version, or for the action to work in private repositories. |
